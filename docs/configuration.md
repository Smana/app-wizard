...
A literal `{` or `}` — for example JSON embedded in a query string — must be
percent-encoded as `%7B` / `%7D`. Percent-encoded braces are not treated as
placeholders: they pass through unchanged, which is exactly what makes them
the escape hatch for a literal brace.
